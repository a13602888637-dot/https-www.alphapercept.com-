...
import { ErrorSeverity, ErrorType } from "@/lib/error/types"
import {
  Wallet,
  TrendingUp,
  BarChart3,
...
export default function PortfolioPageMigrated() {
  ...
  const loadWatchlist = useCallback(async () => {
    ...
    // 使用safeFetch，错误会自动处理
    const { data, error: fetchError, isFallback } = await safeFetch('/api/watchlist', {
      fallbackData: { watchlist: [] } // 提供降级数据
    })

    if (fetchError) {
      // 错误已自动处理（toast + 日志 + 上报）
      // 我们只需要更新本地错误状态
      setError(fetchError.userFriendlyMessage || fetchError.message)
      ...
      await handleError(apiError, 'watchlist_fetch')
      setError(data?.error || "获取自选股失败")
    }

    setWatchlistLoading(false)
  }, [safeFetch, createError, handleError])
  ...
  const handleAiAnalysis = async () => {
    ...
    // 使用API客户端，错误会自动处理
    const { data, error: fetchError, isFallback } = await api.get('/api/analyze-watchlist')

    if (fetchError) {
      // 错误已自动处理
      setAiError(fetchError.userFriendlyMessage || fetchError.message)
  ...
  const handleSearch = async (query: string) => {
    ...
    // 使用safeFetch，错误会自动处理
    const { data, error: fetchError, isFallback } = await safeFetch(
      `/api/stocks/search?q=${encodeURIComponent(query)}`,
      {
        fallbackData: { data: [] } // 提供降级数据
      }
    )
  ...
  const handleDirectAdd = async (query: string) => {
    ...
    try {
      // 使用API客户端
      const { data, error: fetchError } = await api.get(`/api/stocks/search?q=${encodeURIComponent(query)}`)

      if (fetchError) {
        // 错误已自动处理
        toast({
          title: "搜索失败",
          description: fetchError.userFriendlyMessage || fetchError.message,
          variant: "destructive",
        })
        setError(fetchError.userFriendlyMessage || fetchError.message)
        return
  ...
  const handleAddToWatchlist = async (stock: StockResult) => {
    ...
    // 使用API客户端，错误会自动处理
    const { data, error: fetchError } = await api.post('/api/watchlist', {
  ...
  const handleRemoveFromWatchlist = async (id: string) => {
    ...
    // 使用API客户端，错误会自动处理
    const { data, error: fetchError } = await api.delete(`/api/watchlist?id=${id}`)
  ...
  // 创建标准化错误对象用于ErrorDisplay
  const getStandardError = useCallback((): any => {
  ...
  const getAiStandardError = useCallback((): any => {
                            ...
                            onClick={async () => {
                              ...
                              try {
                                for (const item of watchlist) {
                                  await api.delete(`/api/watchlist?id=${item.id}`)
                                }
