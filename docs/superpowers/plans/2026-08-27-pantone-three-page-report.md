...
**Architecture:** 继续使用 1080×1350 PDFKit 画布和现有三页结构。策展层固定热点 3+6、个股 15+15、游资最多 20；渲染层先用 `heightOfString` 测量，若一页容不下则减少最低优先级条目，不新增页面、不截断文字。
...
- 热点最多 3 条未来事件和 6 条新闻；个股最多 30 只；游资最多 20 个。
- 水印和免责声明每页保留。
