...
import { Plus, Search, Filter, RefreshCw, Wifi, WifiOff } from "lucide-react";
import { WatchlistCardWithRealtime } from "./WatchlistCardWithRealtime";
import { toast } from "sonner";
import { useRealTimeStockPrices, StockPrice } from "../../hooks/useRealTimeStockPrices";
...
export function WatchlistManagerWithRealtime() {
  ...
  // Get symbols from watchlist for real-time subscription
  const watchlistSymbols = watchlist.map(item => item.stockCode);
  ...
  const realTimePrices = useRealTimeStockPrices({
    ...
    onPriceUpdate: (update) => {
      // Update local stock prices state
      const updatedPrices: StockPriceMap = {};

      Object.entries(update.prices).forEach(([symbol, priceData]) => {
    ...
    onConnectionChange: (connected) => {
      if (connected) {
        ...
        console.log('Real-time price stream disconnected');
      }
    },
    onError: (error) => {
      console.error('Real-time price error:', error);
      toast.error(`实时价格更新失败: ${error.message}`);
    }
  });
  ...
  const fetchWatchlist = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // Initialize stock prices with buy prices or defaults
        const initialPrices: StockPriceMap = {};
        data.watchlist.forEach((item: WatchlistItem) => {
          initialPrices[item.stockCode] = {
  ...
  // Calculate summary statistics with real-time prices
  const calculateSummary = () => {
    ...
    const currentValue = watchlist.reduce((sum, item) => {
      const priceData = stockPrices[item.stockCode];
      const currentPrice = priceData?.price || item.buyPrice || 0;
    ...
    // Calculate average target profit
  ...
  if (isLoading) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>自选股管理 (实时价格)</CardTitle>
      ...
      <Card>
        <CardHeader>
          <div className="flex justify-between items-center">
            <div>
              <CardTitle>自选股管理 (实时价格)</CardTitle>
              <CardDescription>
                管理您的自选股票，实时价格更新，设置买入成本、止损价和目标价
              </CardDescription>
            </div>
              ...
              <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-gray-100 dark:bg-gray-800">
                {realTimePrices.connected ? (
                  <>
                    ...
                    <span className="text-xs text-green-600 dark:text-green-400">实时连接</span>
                  </>
                ) : (
                  <>
                    <WifiOff className="h-4 w-4 text-red-500" />
                    <span className="text-xs text-red-600 dark:text-red-400">离线</span>
                  </>
                )}
              </div>
        ...
        <CardContent>
          <div className="space-y-4">
            {/* Summary Stats with real-time values */}
            <div className="grid grid-cols-4 gap-4">
              ...
              <Card>
                <CardContent className="pt-6">
                  ...
                  <div className="text-sm text-muted-foreground">当前市值</div>
                  <div className="text-xs text-muted-foreground">
                    投资: ¥{summary.totalInvested.toFixed(2)}
                  </div>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="pt-6">
                  <div className={`text-2xl font-bold ${summary.profitLossPercent >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                    {summary.profitLossPercent >= 0 ? '+' : ''}{summary.profitLossPercent.toFixed(2)}%
                  </div>
                  <div className="text-sm text-muted-foreground">实时盈亏</div>
                  <div className={`text-xs ${summary.totalProfitLoss >= 0 ? 'text-green-600' : 'text-red-600'}`}>
            ...
            {!realTimePrices.connected && !realTimePrices.loading && (
              <div className="p-3 bg-yellow-50 dark:bg-yellow-900/20 rounded-md">
                <div className="flex items-center gap-2 text-sm text-yellow-700 dark:text-yellow-400">
                  <WifiOff className="h-4 w-4" />
                  <span>实时价格连接已断开，正在尝试重新连接...</span>
                </div>
              </div>
            )}
            ...
            {filteredWatchlist.length === 0 ? (
              ...
              <div className="space-y-4">
                {filteredWatchlist.map((item) => (
                  <WatchlistCardWithRealtime
                    key={item.id}
                    item={item}
                    onUpdate={handleUpdateItem}
                    onDelete={handleDeleteItem}
                    priceData={stockPrices[item.stockCode]}
                    isRealTime={realTimePrices.connected}
                  />
                ))}
              </div>
            )}
